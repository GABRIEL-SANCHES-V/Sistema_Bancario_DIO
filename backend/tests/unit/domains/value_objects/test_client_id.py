...
def test_create_client_id_from_uuid():
    cid = ClientID(UUID_1)
...
def test_create_client_id_from_string():
    cid = ClientID(UUID_1_STR)

    assert cid.value == UUID_1
    assert cid.hex == UUID_1.hex


def test_create_client_id_auto_generation():
    cid = ClientID()
...
def test_invalid_type_raises_exception(value):
    with pytest.raises(ClientIdInvalidTypeError):
        ClientID(value)
...
def test_client_id_equality():
    cid1 = ClientID(UUID_1)
    cid2 = ClientID(UUID_1_STR)

    assert cid1 == cid2


def test_client_id_inequality():
    cid1 = ClientID(UUID_1)
    cid2 = ClientID(UUID_2)

    assert cid1 != cid2


def test_client_id_not_equal_other_type():
    cid = ClientID(UUID_1)
...
def test_client_id_set_behavior():
    cid1 = ClientID(UUID_1)
    cid2 = ClientID(UUID_1_STR)
...
def test_client_id_is_immutable():
    cid = ClientID(UUID_1)
...
def test_repr_contains_class_name():
    ...
    assert "ClientID(" in representation
...
@given(st.uuids())
def test_client_id_never_breaks_with_random_uuids(uuid_value):
    cid = ClientID(uuid_value)
...
@given(st.uuids())
def test_client_id_equality_property(uuid_value):
    cid1 = ClientID(uuid_value)
    cid2 = ClientID(str(uuid_value))
